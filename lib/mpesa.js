  ...
  return fetch(
    ...
    .then(function (r) {
      return r.text().then(function (text) {
        if (!r.ok) {
          throw new Error(
            "OAuth HTTP " + r.status + ": " + text.slice(0, 300)
          );
        }
        return safeJson(text, "OAuth");
      });
    })
    .then(function (j) {
...
function stkPush(opts) {
  ...
  const body = {
    ...
    AccountReference: opts.accountRef || (opts.rentId ? "R" + opts.rentId : "JUJO"),
  ...
  return getAccessToken().then(function (token) {
    return fetch(darajaBase() + "/mpesa/stkpush/v1/processrequest", {
      ...
      return r.text().then(function (text) {
        if (!r.ok) {
          throw new Error("STK HTTP " + r.status + ": " + text.slice(0, 300));
        }

        const j = safeJson(text, "STK");
        if (j.ResponseCode !== "0") {
          return {
            ok: false,
