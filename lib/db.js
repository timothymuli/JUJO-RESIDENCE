...
function migratePaymentPerms(d) {
  const uc = tableColumns(d, "users");
  if (!uc.includes("can_confirm_payments")) {
    d.exec(
      "ALTER TABLE users ADD COLUMN can_confirm_payments INTEGER NOT NULL DEFAULT 0"
    );
  }
}
...
function syncTeamMembers(d) {
  ...
  const members = [
    ...
    },
    {
      ...
      staff_title: "Landlord / accountant",
      ml: 1,
      sy: 1,
      superadmin: 0,
      can_confirm: 1,
    },
    {
      ...
      staff_title: "Caretaker — Mlolongo (Paybill)",
      ml: 1,
      sy: 0,
      superadmin: 0,
      can_confirm: 1,
    ...
    },
    {
      ...
      staff_title: "Caretaker — Syokimau (Paybill)",
      ml: 0,
      sy: 1,
      superadmin: 0,
      can_confirm: 1,
  ...
  const upsert = d.prepare(
    `INSERT INTO users (
       email, password_hash, role, property_id, full_name, phone, approval_status,
       staff_title, can_access_mlolongo, can_access_syokimau, is_superadmin, can_confirm_payments
     ) VALUES (?, ?, ?, NULL, ?, ?, 'active', ?, ?, ?, ?, ?)
