...
function seedStaff(d) {
  const staffPass = process.env.STAFF_PASSWORD || "staff123";
  const hash = bcrypt.hashSync(staffPass, 10);

  const mlPhones = (process.env.CARETAKER_SMS_MLOLONGO || "+254745909444,+254727731996")
    .split(/[,;\s]+/)
    .filter(Boolean);
  const syPhones = (process.env.CARETAKER_SMS_SYOKIMAU || "+254723161584,+254785562411")
    .split(/[,;\s]+/)
    .filter(Boolean);

  const members = [
    {
      email: "landlord@jujo.local",
      role: "landlord",
      full_name: "JUJO Property Owner",
      phone: null,
    ...
    {
      email: "caretaker.ml1@jujo.local",
      role: "caretaker",
      full_name: "Mlolongo Caretaker 1",
      phone: mlPhones[0] || "+254745909444",
      staff_title: "Caretaker — Mlolongo",
      ml: 1,
      sy: 0,
    },
    {
      email: "caretaker.ml2@jujo.local",
      role: "caretaker",
      full_name: "Mlolongo Caretaker 2",
      phone: mlPhones[1] || "+254727731996",
      staff_title: "Caretaker — Mlolongo",
      ml: 1,
      sy: 0,
    },
    {
      email: "caretaker.sy1@jujo.local",
      role: "caretaker",
      full_name: "Syokimau Caretaker 1",
      phone: syPhones[0] || "+254723161584",
      staff_title: "Caretaker — Syokimau",
      ml: 0,
      sy: 1,
    },
    {
      email: "caretaker.sy2@jujo.local",
      role: "caretaker",
      full_name: "Syokimau Caretaker 2",
      phone: syPhones[1] || "+254785562411",
      staff_title: "Caretaker — Syokimau",
      ml: 0,
      sy: 1,
    },
  ...
  const exists = d.prepare("SELECT id FROM users WHERE email = ?");

  members.forEach(function (m) {
    if (exists.get(m.email)) return;
    ins.run(
      m.email,
      hash,
      m.role,
