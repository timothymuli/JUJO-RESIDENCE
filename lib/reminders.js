...
function rentReminderText() {
  return (
    ...
    "JUJO Residence: Rent and water for this month are due. Please pay by the 10th. Open your tenant portal and tap Pay now, or use M-Pesa Paybill."
  );
}
...
function startReminders(getDb, sendSms) {
  ...
  // 09:00 on the 1st — rent + water reminder (deadline 10th)
  cron.schedule(
    "0 9 1 * *",
    function () {
      const db = getDb();
      ...
      const template = rentReminderText();
...
module.exports = { startReminders, rentReminderText };
